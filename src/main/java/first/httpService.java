...
public class httpService {
    ...
    /**
     * getMetadataHistory retrives all action made on the metadata ids contained in a list
     * @param functionMetadatas
     * @return Return the Json replied by the First Server.
     * @throws JSONException
     */
    public JSONObject getMetadataHistory(ArrayList<functionMetadata> functionMetadatas) throws JSONException {
        try {
            // Set the API Rest Contact
            URL url = new URL(this.apiUrl + "metadata/history/" + this.apiKey);

            // HTML body builder
            ArrayList<String> metadatas = new ArrayList<String>();
            for (functionMetadata i : functionMetadatas) { metadatas.add("\"" + i.id+ "\"");}
            out.println(metadatas);
            String body = "metadata=" + metadatas;
            HttpsURLConnection conn = this.configPost(url, body);
     ...
     * @param md5 MD5 Hash from the function.
     * @param crc32 CRC32 Hash from the function.
     * @param id ID to add to the metadata
     * @return Return the Json replied by the First Server.
     * @throws JSONException
     */
    public JSONObject applyMetadata(String md5, String crc32, String id) throws JSONException {
        try {
            // Set the API Rest Contact
            URL url = new URL(this.apiUrl + "metadata/applied/" + this.apiKey);

            // HTML body builder
            String body = "md5=" + md5 + "&crc32=" + crc32 + "&id=" + id;
            HttpsURLConnection conn = this.configPost(url, body);
     ...
     * @throws JSONException
     */
    public JSONObject unapplyMetadata(String md5, String crc32, String id) throws JSONException {
        try {
            // Set the API Rest Contact
            URL url = new URL(this.apiUrl + "metadata/unapplied/" + this.apiKey);
    ...
    private JSONObject returnResponse(HttpsURLConnection connection) throws IOException, JSONException {
        ...
        if (connection.getResponseCode() != 200) {
            ...
            String err = "";
            int ch;
            while ((ch = errorStream.read()) != -1) { err += (char) ch;}
            ...
            String obj = "";
            int ch;
            while ((ch = reader.read()) != -1) { obj += (char) ch;}
    ...
    public static void main(String[] args) throws Exception {
        ...
        ArrayList<functionMetadata> functionMetadatas = new ArrayList<functionMetadata>();
