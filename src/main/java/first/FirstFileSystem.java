...
@FileSystemInfo(type = "fstypegoeshere", // ([a-z0-9]+ only)
		description = "File system description goes here", factory = FirstFileSystem.MyFileSystemFactory.class)
public class FirstFileSystem implements GFileSystem {
	 ...
	 */
	public FirstFileSystem(FSRLRoot fsFSRL, ByteProvider provider) {
		this.fsFSRL = fsFSRL;
	...
	public void mount(TaskMonitor monitor) {
		monitor.setMessage("Opening " + FirstFileSystem.class.getSimpleName() + "...");
	...
	public static class MyFileSystemFactory
			implements GFileSystemFactoryFull<FirstFileSystem>, GFileSystemProbeFull {

		@Override
		public FirstFileSystem create(FSRL containerFSRL, FSRLRoot targetFSRL,
				ByteProvider byteProvider, File containerFile, FileSystemService fsService,
				TaskMonitor monitor) throws IOException, CancelledException {

			FirstFileSystem fs = new FirstFileSystem(targetFSRL, byteProvider);
			fs.mount(monitor);
			return fs;
		}
